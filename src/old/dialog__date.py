...
class DateRangeDialog(QDialog):
    ...
    def __init__(self, parent, search_word, is_minus=False):
    ...
    def get_search_text(self):
        ...
        if self.is_minus:
            if upper > 1:
                stext = f"(-{self.search_word}:{lower} OR {self.search_word}:{upper - 1})"
            else:
                stext = f"-{self.search_word}:{lower}"
        else:
            stext = f"{self.search_word}:{lower}"
            if upper > 1:
                stext += f" -{self.search_word}:{upper - 1}"
        return stext
