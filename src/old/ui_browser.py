...
from .. import load_new_conf
def switch_to_new_version():
    if not askUser("Really switch to new version of this addon?"):
        return
    # store current config in user files + write old config meta.json
    load_new_conf()
    msg = ("Restart Anki now. Otherwise you'll run into error messages. The new config "
           "has been loaded. The old and new configs are independent of each other. So "
...
def setup_browser_menu(self):
    ...
    action = QAction(self)
    action.setText("Switch to new add-on version")
    bs_menu.addAction(action)
    action.triggered.connect(lambda _: switch_to_new_version())
