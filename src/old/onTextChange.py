...
def range_dialog(parent, search_operator, user_string, before, after, chars_to_del, func_settext):
    is_minus = before.endswith(f"-{user_string}") 
    d = DateRangeDialog(parent, search_operator, is_minus)
    if d.exec():
        ...
        remove_these = chars_to_del+1 if is_minus else chars_to_del
        new_text = before[:-remove_these] + spaces + d.searchtext + after
        func_settext(new_text)
        newpos = len(before[:-chars_to_del] + spaces + d.searchtext)
...
def onSearchEditTextChange(parent, 
    ...
    if da_match:
        anki_search_operator = "added"
        length = len(da)
        user_string = da
    if de_match:
        anki_search_operator = "edited"
        length = len(de)
        user_string = de
    if dr_match:
        anki_search_operator = "rated"
        length = len(dr)
        user_string = dr
    if di_match:
        anki_search_operator = "introduced"
        length = len(di)
        user_string = di
    if any([da_match, de_match, dr_match, di_match]):
        return range_dialog(parent, anki_search_operator, user_string, before, after, length, func_settext)
