...
def get_default_conf_for_this_addon__old_new(addon_abs_path):
    path = os.path.join(addon_abs_path, "config.json")
    try:
...
def gui_config_helper(
    ...
    ## old-new
    default_conf = get_default_conf_for_this_addon(addon_abs_path)
    default_conf = get_default_conf_for_this_addon__old_new(addon_abs_path)
